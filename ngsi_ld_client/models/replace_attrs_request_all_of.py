...
REPLACEATTRSREQUESTALLOF_ONE_OF_SCHEMAS = ["GeoProperty", "LanguageProperty", "ModelProperty", "Relationship"]

class ReplaceAttrsRequestAllOf(BaseModel):
    """
    ...
    actual_instance: Any
    one_of_schemas: List[str] = Field(REPLACEATTRSREQUESTALLOF_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ReplaceAttrsRequestAllOf.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ReplaceAttrsRequestAllOf with oneOf schemas: GeoProperty, LanguageProperty, ModelProperty, Relationship. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ReplaceAttrsRequestAllOf with oneOf schemas: GeoProperty, LanguageProperty, ModelProperty, Relationship. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> ReplaceAttrsRequestAllOf:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> ReplaceAttrsRequestAllOf:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ReplaceAttrsRequestAllOf with oneOf schemas: GeoProperty, LanguageProperty, ModelProperty, Relationship. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ReplaceAttrsRequestAllOf with oneOf schemas: GeoProperty, LanguageProperty, ModelProperty, Relationship. Details: " + ", ".join(error_messages))
        else:
            return instance
