...
|-------------|-------------|------------------|
**201** | The HTTP response shall include a \&quot;Location\&quot; HTTP header that contains the resource URI of the created subscription resource.  |  * NGSILD-Tenant -  <br>  * Location -  <br>  * NGSILD-Warning -  <br>  |
...
> QuerySubscription200ResponseInner retrieve_subscription(subscription_id, options=options, local=local, link=link, ngsild_tenant=ngsild_tenant)
...
with ngsi_ld_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Subscription retrieval by id 
        api_response = api_instance.retrieve_subscription(subscription_id, options=options, local=local, link=link, ngsild_tenant=ngsild_tenant)
